...
extension EarthquakeHomeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.annotate(annotation: Annotation(title: "",
                                             latitude: viewModel.getEarthquakeData()?[indexPath.row].geometry.coordinates[1] ?? 0.0,
                                             longitude: viewModel.getEarthquakeData()?[indexPath.row].geometry.coordinates[0] ?? 0.0))
        return cell
    }
